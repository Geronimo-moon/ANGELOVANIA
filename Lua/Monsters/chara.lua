...
function SetLang()
  if Encounter.GetVar("japanese") == true then
    messages = {
      ...
      c31 = {'[func:Stop]...','[font:det_jp_mini]くらった、だと?','[font:det_jp_mini][func:CharaHead,chara/damnhead]ダメだ...\nそんなわけには...','[font:det_jp_mini][func:CharaHead,chara/downhead][noskip]かれがくるまでに\nおわらせないと[next]'},
    }
  else
    messages = {
...
-- プレイヤーが攻撃したとき
  if damage == -1 and not turn == 30 then
    ...
    elseif turn == 31 then
      ...
      CharaBody('chara/woundbody')
      currentdialogue = messages.c31
      Encounter.SetVar("nextwaves",{"P3/anime"})
    end
  end
end
